...
class DateOfBirthComparator implements Comparator<Q07TP2.personagem> {

  @Override
  public int compare(Q07TP2.personagem p1, Q07TP2.personagem p2) {
    // Converta as datas de nascimento para objetos Date para facilitar a comparação
    Date dateOfBirth1 = parseDate(p1.getDateOfBirth());
    Date dateOfBirth2 = parseDate(p2.getDateOfBirth());
    // Compare as datas de nascimento
    int dateComparison = dateOfBirth1.compareTo(dateOfBirth2);
    // Se as datas de nascimento forem iguais, compare pelos nomes
    if (dateComparison == 0) {
  ...
  private Date parseDate(String dateOfBirthString) {
    ...
    try {
      return format.parse(dateOfBirthString);
...
public class Q07TP2 {
  ...
  public static void main(String[] args) {
    ...
    List<personagem> PersonagensFornecidos = new ArrayList<>();
    String entrada = sc.nextLine();
    while (!(entrada.equals("FIM"))) {
      for (personagem p : personagens) {
        if (p.getId().equals(entrada)) {
          PersonagensFornecidos.add(p);
          break;
        }
      }
      entrada = sc.nextLine();
    }
    Collections.sort(PersonagensFornecidos, new DateOfBirthComparator());

    for (personagem personagem : PersonagensFornecidos) {
      System.out.println(personagem);
    }
  }
}
